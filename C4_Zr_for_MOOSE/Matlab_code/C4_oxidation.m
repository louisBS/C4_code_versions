...
% Create gradients at interfaces vectors
grad_ba = zeros(1,N+1);
grad_ab = zeros(1,N+1);
grad_aox = zeros(1,N+1);
...
% Gradients values : no alpha beta interface yet but some oxide
grad_aox(n) = (Co(K+2,n)-Co(K+1,n))/dx_moving;
grad_ab(n) = NaN ;
grad_ba(n) = NaN ;

init = n + 1;       % Custom starting iteration depending if we used a preoxide or not
                    % init = 3 if no preoxide ; 2 if preoxide
...
%             Db = 0.11 * exp(-33390./(RR*T(n)));            % UW-Madison (1000C --> 1500C)
    ...
    if dx_a == 0
        ...
        % Below phase transition : still no alpha/beta interface
        grad_aox(n) = (Co(K+2,n)-Co(K+1,n))/dx_moving;
        grad_ab(n) = NaN ;
        grad_ba(n) = NaN ;
...
%                 Jh(n) = 2 * fh_inst(n) * Jv(n);         % If we already know the hydrogen pickup (from experiment)
            ...
            % alpha/beta interface created
            grad_aox(n) = (Co(K+2,n)-Co(K+1,n))/dx_a;
            grad_ab(n) = (Co(K_fixed+K_b+3,n)-Co(K_fixed+K_b+2,n))/dx_a ;
            grad_ba(n) = (Co(K_fixed+K_b+1,n)-Co(K_fixed+K_b,n))/dx_b ;
...
%                     Jh(n) = 2 * fh_inst(n) * Jv(n);         % If we already know the hydrogen pickup (from experiment)
            ...
            % Calculate the gradients at the interfaces
            grad_aox(n) = (Co(K+2,n)-Co(K+1,n))/dx_a;
            %grad_ab(n) = (Co(K_fixed+K_b+3,n)-Co(K_fixed+K_b+2,n))/dx_a ;
            grad_ab(n) = (Co(K_fixed+K_b+3,n)-Co_a_b)/dx_a ;
            %grad_ba(n) = (Co(K_fixed+K_b+1,n)-Co(K_fixed+K_b,n))/dx_b ;
            grad_ba(n) = (Co_b_a-Co(K_fixed+K_b,n))/dx_b ;
            ...
            % Calculate the gradients at interfaces
            grad_aox(n) = (Co(K+2,n)-Co(K+1,n))/dx_a;
            grad_ab(n) = (Co(K_fixed+K_b+3,n)-Co(K_fixed+K_b+2,n))/dx_a ;
            grad_ba(n) = (Co(K_fixed+K_b+1,n)-Co(K_fixed+K_b,n))/dx_b ;
        end
